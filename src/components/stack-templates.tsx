  ...
  if (!templates) {
    return (
      <div className="text-center p-4">
        <div className="text-muted-foreground text-sm pixel-loading">LOADING TEMPLATES...</div>
      </div>
    );
  }
    ...
    <div className={cn("space-y-6", className)}>
      ...
      {templates.length === 0 ? (
        <PixelCard className="p-8 text-center">
          ...
          <p className="text-muted-foreground text-xs">Try a different category</p>
        </PixelCard>
      ) : (
    ...
    >
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-primary text-base">{template.name}</h3>
      ...
      {templates.map((template: any) => (
        <Link key={template._id} href={`/templates/${template.slug}`}>
          <PixelCard className="p-3 hover:border-primary transition-colors">
            <p className="text-primary text-sm">{template.name}</p>
            <p className="text-muted-foreground text-xs">{template.tools.length} tools</p>
          </PixelCard>
        </Link>
      ))}
    </div>
