  ...
  if (!evolution) {
    return (
      <div className="text-center p-4">
        <div className="text-muted-foreground text-sm pixel-loading">LOADING...</div>
      </div>
    );
  }
  ...
  return (
    <div className={cn("space-y-4", className)}>
      <PixelCard className={cn("p-4", tierConfig.color, tierConfig.bg)}>
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-primary text-base">{evolution.tool?.name}</h3>
              <PixelBadge variant="outline" className={cn("text-xs", tierConfig.color)}>
      ...
      {history && history.length > 0 && (
        <div>
          <h4 className="text-primary text-sm uppercase mb-2 flex items-center gap-2">
              ...
              return (
                <div key={e._id} className={cn("flex items-center gap-3 p-2 border", config.color)}>
                  ...
                  <div className="flex-1">
                    <span className="text-primary text-sm">{e.tier.toUpperCase()}</span>
          ...
          return (
            <Link key={e._id} href={`/tools/${e.tool?.slug}`}>
              <div className={cn("flex items-center gap-2 p-2 border hover:border-primary", config.color)}>
                <config.icon className={cn("w-4 h-4", config.color.split(" ")[0])} />
                <span className="text-primary text-sm flex-1">{e.tool?.name}</span>
        ...
        {(Object.entries(TIER_CONFIG) as [keyof typeof TIER_CONFIG, typeof TIER_CONFIG.bronze][]).map(([tier, config]) => {
          ...
          return (
            <div key={tier} className="flex items-center gap-2">
              ...
              <span className="text-muted-foreground text-xs w-8">{count}</span>
            </div>
          );
        })}
      </div>
          ...
          return (
            <Link key={badge._id} href={`/tools/${badge.tool?.slug}`}>
              <div className={cn("p-2 border text-center", currentConfig.color, currentConfig.bg)}>
                ...
                <p className="text-primary text-xs mt-1">{badge.tool?.name}</p>
